...
class CbfSpider(scrapy.Spider):
    ...
    start_urls = [
        f"https://www.cbf.com.br/amp/futebol-brasileiro/competicoes/campeonato-brasileiro-serie-a/2022/{i+1}" for i in range(1)]

    def parse(self, response):
        numero_jogo = int(response.url.split('/')[-1].split('?')[0])
        data_jogo = response.css('.col-xs-6 span::text').get()
        jogo = {'numero_jogo': numero_jogo, "time_mandante": "Corinthians",
